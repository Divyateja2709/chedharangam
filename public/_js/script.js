...
function isKingInCheck(board, color) {
    ...
    return isUnderAttack(board, color, kingPos);
}
...
function isUnderAttack(board, color, position) {
    for (let row = 0; row < 8; row++) {
        for (let col = 0; col < 8; col++) {
            const piece = board[row][col];
...
function handleMove(from, to) {
    ...
    const currentPlayer = board[to.row][to.col].color;
    const opponent = currentPlayer === 'white' ? 'black' : 'white';

    if (isCheckmate(board, opponent)) {
        alert(`${currentPlayer.charAt(0).toUpperCase() + currentPlayer.slice(1)} wins by checkmate!`);
        updateGameStatus(`${currentPlayer.charAt(0).toUpperCase() + currentPlayer.slice(1)} wins by checkmate!`);
        endGame();
        showGameEndModal();  // Show the modal
    }
}
...
function getPossibleMoves(board, piece, position) {
    return [];
}

function checkForKingPresence(board) {
    ...
    if (!whiteKingAlive) {
        notifyGameEnd("Black wins! White king has been captured.");
        return true;
    }

    if (!blackKingAlive) {
        notifyGameEnd("White wins! Black king has been captured.");
        return true;
    }
...
function notifyGameEnd(message) {
    alert(message);
    disableBoard();
}
...
function handleCapturedPiece(piece) {
    if (piece.type === 'king') {
        updateGameStatus("Checkmate! Game Over.");
        endGame();
        showGameEndModal();  // Show the modal
    }
}
...
// Example: Show the modal when the game ends
function showGameEndModal() {
    $('#gameEndNotification').modal('show'); // Show modal
    // Optionally, hide the game board or update the page content
}

// Add event listener for the "Start New Game" button
document.getElementById('startNewGameBtn').addEventListener('click', function() {
    // Logic for starting a new game, e.g., reset board state
    $('#gameEndNotification').modal('hide'); // Hide modal
    startNewGame(); // Implement the new game logic
});

function startNewGame() {
    // Reset board and game state here
    gameOver = false;
    allPieces = [];
    // Additional logic to reset the board and game state
}
